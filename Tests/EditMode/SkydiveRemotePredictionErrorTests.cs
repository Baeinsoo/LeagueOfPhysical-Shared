...
namespace LOP.Tests
{
    ...
    public class SkydiveRemotePredictionErrorTests
    {
        ...
        public void 자세를_바꾼_창의_오차가_물리가_허용하는_한계_안에_있다()
        {
            ...
            //  다른 축의 숫자를 빌려 쓰는 것이 된다). 자세 슬라이더가 창 내내 올라가는 동안 목표
            //  낙하 속도가 계속 앞서 도망가서, 진실 월드는 갈린 틱 전부를 최대 가속(FallApproach)
            //  으로 떨어진다 — 그래서 오차는 이 천장의 상한이 아니라 <b>같은 값</b>이다.
            //  ⚠️ 연속시간 공식(½at²)이 아니다 — 시뮬은 매 틱 "속도를 먼저 a×dt만큼 올리고 그
            //  새 속도로 위치를 미는" 이산 적분이라, n틱 뒤 누적 거리는 a×dt²×n(n+1)/2다.
            ...
            //  이 여유는 <b>여유를 사려고</b> 넣은 게 아니라 float32 양자화 몫이다 — 위치가
            //  ≈11 m 근처라 한 눈금이 ≈1e-6 m이고, 이 여유는 그 열 배쯤이다.
            const float FloatNoise = 1e-5f;
            Assert.AreEqual(ceiling, error, FloatNoise,
                $"오차 {error:F5}m가 낙하 가속 한계 {ceiling:F5}m와 같지 않다 — " +
                "너무 작으면 자세 변경이 낙하 속도에 영향을 주지 못하는 상태(테스트가 아무것도 재지 못함)이고, " +
                "너무 크면 낙하 가속 말고 다른 데서도 두 월드가 갈렸다는 뜻이다");
        }
