...
namespace LOP
{
    ...
    public static class ArcheryHitTest
    {
                                           ...
                                           float radius, out float t)
        {
            ...
            //  시작점이 판의 어느 쪽에 있나. 아래 cross의 부호가 이 값의 부호와 같으므로,
            //  "판 뒤에서 출발"(음수)은 cross < 0 검사가 이미 걸러낸다.
            //
            //  그래서 이 가드가 실제로 막는 것은 **정확히 판 위에서 출발한** 한 경우다 —
            //  앞 틱에 평면에 딱 닿아 이미 맞은 화살이 다음 틱에 또 맞는 것을 막는다.
            //  ⚠️ 이게 중복으로 보여도 아래 cross < 0 검사를 지우지 마라. 그쪽이 뒤에서
            //  오는 화살을 막는 진짜 검사다.
