namespace LOP
{
    ...
    public class InputCommand
    {
        ...
        /// <summary>
        /// 손가락을 대고 있는 동안 보내는 당김 목표(0 또는 1) — 더 이상 끈 거리가 아니다.
        /// 실제 당김 정도와 화살 속도는 이 목표를 향해 시뮬(<see cref="ArcheryAimSystem"/>)이
        /// 정해진 속도로 올려가며 걸린 틱 수(=시간)로 정해진다. 취소 판정(임계치 미달)도 이
        /// 필드가 아니라 시뮬이 갖고 있는 <c>ArcheryAim.DrawRatio</c>를 본다. 서버는 이 값을
        /// 그대로 믿지 않고 0~1로 clamp한다 — 클라가 임의 크기를 실어도 결과는 시간으로만
        /// 정해지게 하기 위함이다.
        /// </summary>
